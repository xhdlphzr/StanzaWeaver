...
def _build_writer_system(description: str, poem: list[str], template: dict, template_obj=None, feedback: str = "") -> str:
        ...
        for i, count in enumerate(lines_spec):
            ...
            if line_constraints:
                ...
                for j, c in enumerate(line_constraints):
                    ...
                    if desc_parts:
                        constraint_strs.append(f"    第{j+1}位: {','.join(desc_parts)}")
                if constraint_strs:
    ...
    prompt_parts.append("每次调用 refine_line 或 rewrite 后，系统会自动校验格律，不通过会返回具体错误，你需要根据错误调整。")
...
class WriterAI:
    ...
    def generate_description(self, topic: str, on_stream: object = None) -> tuple[str, str]:
        messages = [
            {
                "role": "system",
    ...
    def generate_draft(self, description: str, template: dict, template_obj=None, max_attempts: int = 5, on_stream: object = None) -> tuple[list[str], str]:
        language = template.get("language", "zh")
...
只输出纯诗文本，每行用换行分隔。"""
    ...
    def refine(
        ...
        system_prompt = _build_writer_system(description, current_poem, template, template_obj, feedback)

        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": "请开始炼句优化。先用 search_words 搜候选词，然后必须至少调用一次 refine_line 或 rewrite 修改诗句，才能调用 submit 提交。"},
        ]
        ...
        for _round in range(max_rounds):
            ...
            for tool_call in response["tool_calls"]:
                ...
                if name == "submit":
                    if modifications < 1:
                        detail_parts.append(f"[第{_round+1}轮] submit: 拒绝 - 尚未进行任何修改，必须先调用 refine_line 或 rewrite")
                        result_for_msg = {"error": "不允许直接提交。你必须至少调用一次 refine_line 或 rewrite 成功修改诗句后，才能调用 submit。"}
                        history.append({"tool": "submit", "arguments": args, "result": "rejected_no_changes"})
                        assistant_msg = LLMClient.assistant_to_message(response)
                        tool_msgs = [
                            {"role": "tool", "tool_call_id": tc["id"], "content": json.dumps(result_for_msg, ensure_ascii=False)}
                ...
                if name == "search_words":
                    ...
                    detail_parts.append(f"[第{_round+1}轮] search_words({args.get('meaning','')}): 找到{word_count_result}个候选词")
                elif name == "refine_line":
                    result = execute_refine_line(current_poem, template, args)
                    if "poem" in result:
                        current_poem = result["poem"]
                        modifications += 1
                        full_result = self.validator.validate(current_poem, template, template_obj)
                    ...
                    result = self._handle_rewrite(description, current_poem, template, template_obj, args)
                    if "poem" in result:
                        current_poem = result["poem"]
                        modifications += 1
                        full_result = self.validator.validate(current_poem, template, template_obj)
                        if not full_result.passed:
                            result["validation_errors"] = full_result.errors
                    detail = f"[第{_round+1}轮] rewrite({args.get('instruction','')})"
                    if "poem" in result:
                        detail += ": 重写完成"
                        if "validation_errors" in result:
                            detail += f" (格律问题: {result['validation_errors']})"
        ...
        return current_poem, submitted, history, "\n".join(detail_parts) if detail_parts else ""

    def _handle_rewrite(self, description: str, poem: list[str], template: dict, template_obj=None, args: dict = None) -> dict:
        instruction = args.get("instruction", "")
        lines = template.get("lines", 4)
        syllables_per_line = template.get("syllables_per_line", [])

        sys_prompt = f"""请根据指令重写全诗。
...
请直接输出重写后的全诗，每行一句，不要加序号或其他文字。"""
        ...
        for attempt in range(3):
            ...
            if len(new_poem) != lines:
                messages.append(LLMClient.assistant_to_message(response))
                messages.append({"role": "user", "content": f"输出行数为{len(new_poem)}，需要恰好{lines}行。"})
                continue
