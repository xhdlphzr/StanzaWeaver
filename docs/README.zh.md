...
3. **Step 3 · 炼句循环（ReAct）**：AI 反复调用 `search_words` 检索合规字词、`refine_line` 整行替换、`rewrite` 整体重写；每一次修改都立即触发符号层对全诗的格律校验。AI 可随时调用 `submit`，但提交仅在**全诗格律校验通过后**才被接受——不通过则以具体错误打回并继续炼句。**该循环无轮数上限**，直到以合律诗稿提交定稿。
